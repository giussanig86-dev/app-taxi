...
import {
  ...
  Gauge, User, Users, CheckSquare, Settings, X, Car
} from 'lucide-react'
...
export default function Sidebar({ isOpen, onClose }) {
  const { user, isCliente, isConsulente } = useAuth()
  const links = isCliente ? clienteLinks : isConsulente ? consulenteLinks : clienteLinks
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-5 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-white/50">Forfettario SaaS</p>
            </div>
          </div>
        ...
        <div className="px-5 py-3 border-b border-white/10">
          <p className="text-xs text-white/40 uppercase tracking-wider">
            {user?.ruolo === 'cliente' ? 'Tassista' : 'Consulente'}
          </p>
          ...
          {links.map((link) => {
            ...
            return (
              <NavLink
                key={link.to}
                to={link.to}
                end={link.to === '/dashboard' || link.to === '/consulente'}
                onClick={onClose}
                className={({ isActive }) =>
                  cn(
