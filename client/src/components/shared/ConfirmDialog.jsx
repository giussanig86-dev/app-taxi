...
export default function ConfirmDialog({ isOpen, open, onClose, onCancel, onConfirm, title, message, confirmLabel = 'Elimina', danger = false, variant }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6">
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button
            onClick={onConfirm}
            className={`px-4 py-2 text-sm font-medium text-white rounded-lg ${isDanger ? 'bg-danger hover:bg-red-600' : 'bg-primary hover:bg-primary-dark'}`}
          >
            {confirmLabel}
          </button>
        </div>
